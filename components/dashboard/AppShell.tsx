...
//
// `h-dvh` + `overflow-hidden` is the right model on a laptop — the promise is
// that the dashboard fits one screen. On a phone it forces every list into an
// inner scroll box, so content stops at that box's edge partway down the screen
// instead of at the bottom of the display, and iOS never collapses its address
// bar because the document itself never scrolls. Below `xl` the shell simply
// grows and the page scrolls like a page.
  ...
  return (
    <div className="min-h-dvh xl:h-dvh flex flex-col bg-ground text-ink xl:overflow-hidden">
      ...
      {/* `flex-1` in a `min-h-dvh` column means main fills whatever the nav
          leaves and grows past it when content is taller — no calc guessing at
          the chrome height, which is variable anyway once iOS safe-area insets
          are in play. */}
